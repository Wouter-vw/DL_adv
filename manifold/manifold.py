...
class LinearizedCrossEntropyManifold:
    ...
    def geodesic_system(self, current_point, velocity, return_hvp=False):
        ...
        _, result = self.custom_hvp(
            ...
            (velocity_tree, (jnp.zeros_like(data[0]), jnp.zeros(data[1].shape, dtype=jax.dtypes.float0)), jnp.zeros_like(self.f_MAP))
        )
...
class CrossEntropyManifold:
    ...
    def geodesic_system(self, current_point, velocity, return_hvp=False):
        ...
        hvp_data_fitting = 0
        _, result = self.custom_hvp(
            self.cross_entropy_loss, (params, data), (velocity_tree, (jnp.zeros_like(data[0]), jnp.zeros(data[1].shape, dtype=jax.dtypes.float0)))
        )
